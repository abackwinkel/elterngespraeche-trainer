...
/**
 * Repariert LLM-JSON, in dem ein Zitat INNERHALB eines String-Werts als Mischpaar
 * geschrieben wurde: geöffnet mit „, geschlossen mit geradem " (Befund 22.07.2026,
 * Elterngespräche-Trainer). Das gerade Zeichen beendet den JSON-String vorzeitig —
 * die Antwort ist unparsebar, obwohl die Begrenzer korrekt sind. Genau dieser Fall
 * entgeht repairGermanQuotedJson, das nur falsche BEGRENZER behandelt.
 *
 * Nur auf dem Fehlerpfad anwenden (nach einem gescheiterten JSON.parse): Bei
 * gültigem JSON gibt es nichts zu reparieren, und ein " ohne vorangehendes „
 * bleibt in jedem Fall unberührt.
 */
export function repairMixedQuotesInJson(s) {
  if (s == null) return s;
...
function runSelfTest() {
  ...
  // Mischpaar IM Wert bricht den JSON-String (Befund 22.07.2026): „gemeinsam" -> „gemeinsam“
  const mixed = '{"gut": "Sie sagen „gemeinsam" und das wirkt."}';
  let mixedOk = true;
  try { JSON.parse(mixed); mixedOk = false; } catch { /* erwartet: kaputt */ }
  eq('Mischpaar im Wert ist wirklich kaputt', String(mixedOk), 'true');
  let repairedOk = true, repairedVal = '';
  try { repairedVal = JSON.parse(repairMixedQuotesInJson(mixed)).gut; } catch { repairedOk = false; }
  eq('Mischpaar-Reparatur parsebar', String(repairedOk), 'true');
  eq('Mischpaar-Reparatur inhaltlich', repairedVal, 'Sie sagen „gemeinsam“ und das wirkt.');
  eq('Reparatur laesst gueltiges JSON in Ruhe',
     repairMixedQuotesInJson('{"gut": "ohne Zitat"}'), '{"gut": "ohne Zitat"}');
