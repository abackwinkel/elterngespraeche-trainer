...
export default function QuizPage() {
  ...
  function handleCheck() {
    ...
    // API-Aufruf (fire-and-forget, für Statistik)
    ...
    return (
      <div className="p-6 md:p-10 max-w-3xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {moduls.filter(m => m !== 'koerpersignale').map(modul => {
            const count = QUIZ_FRAGEN.filter(f => f.modul === modul).length
            return (
              <button
                key={modul}
                onClick={() => handleModulSelect(modul)}
                className={`border-2 rounded-xl p-5 text-left hover:scale-[1.02] transition-transform ${MODUL_FARBE[modul] ?? 'bg-white border-[var(--c-gray-light)]'}`}
              >
                <div className="text-base font-semibold mb-1">{MODUL_LABEL[modul]}</div>
